...
- If asked about a product we don't carry, say "We don't currently have that in stock, but let me suggest something similar we do carry."
...
- If asked about store hours, policies, events or specials, use the info below.
...
def serve_glb(): return send_file(os.path.join(os.path.dirname(__file__),'static','assets','avatar.glb'), mimetype='model/gltf-binary', conditional=True, etag=True, max_age=86400)
